...
if(isset($_POST['username'])) {
    if($stm = $connect -> prepare('UPDATE users set username = ?, email = ?, active = ? WHERE ID = ?')) {
        ...
        if(isset($_POST['password'])) {
            ...
            set_message("The user id " . $_GET['id'] . " has been updated");
...
<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-md-6">
            <h1 class="h1-display-h1">Edited user</h1>
             ...
             <form method="post">
                ...
                <div class="form-outline mb-4">
                    <select name="active" class="form-select" id="active">
                        <option <?php echo ($user['acitve']) ? "selected" : ""; ?> value="1">Active</option>
                        <option <?php echo ($user['acitve']) ? "" : "selected"; ?> value="0">Inactive</option>
                    </select>
                </div>

                <!-- Submit button -->
                <button type="submit" class="btn btn-primary btn-block">Updated user</button>
            </form>
